...
impl MusicLibrary
{
	pub fn new(cacheFile: &Path, basePath: &Path) -> Result<Arc<RwLock<Self>>>
	{
		if cacheFile.exists()
		{
			Self::fromCache(cacheFile).or_else(|report| {
	...
	pub fn fromPath(cacheFile: &Path, basePath: &Path) -> Result<Arc<RwLock<Self>>>
	{
		...
		let library = Arc::new(RwLock::new(Self {
	...
	fn backgroundDiscover(
		localLibrary: &Arc<RwLock<Self>>, library: Arc<RwLock<Self>>, currentDirectory: PathBuf,
	) -> Result<()>
	{
		let task = async move { Self::discover(library.as_ref(), currentDirectory.as_path()) };
	...
	fn writeLock(library: &RwLock<Self>) -> Result<RwLockWriteGuard<'_, Self>>
	{
		library
			.write()
			.map_err(|error| eyre::eyre!("While discovering library: {}", error))
	}
		...
		for entry in contents
		{
			...
			if path.is_dir()
			{
				let relativePath = path
					.strip_prefix(&Self::readLock(library)?.basePath)?
					.to_path_buf();
				Self::writeLock(library)?
					.dirs
					.insert(relativePath.clone());
				...
				if !Self::readLock(library)?.files.contains_key(&path) &&
					!Self::readLock(library)?
						.dirs
						.iter()
						.any(|dir| dir.starts_with(&relativePath) && dir != &relativePath)
				{
			...
			else
			{
				...
				if !Self::readLock(library)?
					.files
					.contains_key(filePath)
				{
					Self::writeLock(library)?
						.files
						.insert(filePath.to_path_buf(), BTreeSet::new());
				}
